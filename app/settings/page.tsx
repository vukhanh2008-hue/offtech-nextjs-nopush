...
export default function Settings(){
  ...
  async function saveQuiet(){
    ...
    const { error } = await supabase.from('user_settings').upsert({
      user_id: user.id, quiet_start: start, quiet_end: end
    });
    if(error) alert(error.message); else alert('Đã lưu quiet hours');
  }
  ...
  return (
    <main style={{padding:16}}>
      ...
      <div><h3>Quiet hours</h3>
        ...
        <button onClick={saveQuiet} style={{marginLeft:8}}>Lưu</button>
      </div>
      <div style={{marginTop:16}}><h3>Đổi mật khẩu</h3>
        ...
        <button onClick={changePwd} style={{marginLeft:8}}>Đổi</button>
      </div>
    </main>
  );
}
